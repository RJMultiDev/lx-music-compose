...
/**
 * Create navigation state that persists across config changes and process death.
 * Adapted from official Navigation 2 → 3 migration guide.
 */
@Composable
...
class NavigationState(
    ...
    fun currentStack(): NavBackStack<NavKey> =
        backStacks[topLevelRoute] ?: error("Stack for $topLevelRoute not found")

    fun currentEntry(): NavKey = currentStack().last()
}
