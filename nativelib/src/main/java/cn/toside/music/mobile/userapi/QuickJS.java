...
public class QuickJS {
  ...
  private long loadScriptTime = 0;

  public QuickJS(Context context, Handler eventHandler) {
  ...
  private String getPreloadScript() {
    try {
      InputStream inputStream = this.appContext.getAssets().open("script/user-api-preload.js");
  ...
  public String loadScript(Bundle scriptInfo) {
    loadScriptTime = System.currentTimeMillis();
  ...
  public Object callJS(Object[] params) {
    try {
      ...
      // Always send INIT_FAILED for UnhandledPromiseRejection during the init window,
      // even if inited was already set (the error may have occurred during the same call)
      else if (msg.contains("UnhandledPromiseRejectionException")) {
        long elapsed = System.currentTimeMillis() - loadScriptTime;
        if (elapsed < 10000) {
          Log.w("UserApi", "Sending INIT_FAILED for post-init unhandled rejection (elapsed=" + elapsed + "ms)");
          eventHandler.sendMessage(eventHandler.obtainMessage(HandlerWhat.INIT_FAILED, msg));
        }
      }
      return null;
